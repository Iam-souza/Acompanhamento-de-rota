...
class SupabaseDB:
    def __init__(self):
        ...
        self.url = "https://bekjfhrfpmuxrgcsbyns.supabase.co"
        self.key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJla2pmaHJmcG11eHJnY3NieW5zIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjEzMjM2MTYsImV4cCI6MjA3Njg5OTYxNn0.70A7KcikrTLGZMZO3APTnURNfYXOL2nB6Q4cbgprPBE"
    ...
    def insert_relatorio_raw(self, data: Dict) -> bool:
        """Insere dados brutos do relatório"""
        try:
            response = self.client.table(self.table_relatorios_raw).insert(data).execute()
            return True
        except Exception as e:
    ...
    def insert_relatorio_consolidado(self, data: Dict) -> bool:
        """Insere dados consolidados (evita duplicatas)"""
        try:
            # Verifica se já existe registro com mesmo contrato e OS
            ...
            if not existing.data:
                response = self.client.table(self.table_relatorios_consolidados).insert(data).execute()
                return True
            else:
                # Já existe, não insere duplicata
                return False
        except Exception as e:
    ...
    def get_relatorios_consolidados(self, filters: Dict = None) -> pd.DataFrame:
        """Busca relatórios consolidados com filtros opcionais"""
        try:
            ...
            if response.data:
                ...
                # Converte data_cadastro para datetime se existir
    ...
    def update_relatorio_consolidado(self, record_id: str, updates: Dict) -> bool:
        """Atualiza registro consolidado"""
        try:
            # Adiciona timestamp de atualização
            updates['updated_at'] = 'now()'
            
            response = self.client.table(self.table_relatorios_consolidados)\
                .update(updates)\
                .eq("id", record_id)\
                .execute()
            return True
    ...
    def get_unique_values(self, column: str, table: str = None) -> List[str]:
        """Busca valores únicos de uma coluna para filtros"""
        try:
            if table is None:
            ...
            if response.data:
                values = [item[column] for item in response.data if item[column] is not None]
    ...
    def consolidate_data(self, raw_data: Dict) -> Dict:
        """Extrai dados relevantes para consolidação"""
        consolidated = {
            ...
            "acao_trativa": None,
            # Campos editáveis inicializados como None
            "log": None,
            "certidao": None,
            "ativo": None,
            "tratativa": None,
    ...
    def bulk_consolidate_from_raw(self) -> int:
        """Consolida todos os dados brutos que ainda não foram consolidados"""
        try:
